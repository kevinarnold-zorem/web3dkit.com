...
import { createReadStream, existsSync, realpathSync, type Stats } from "node:fs";
...
interface MirrorOptions {
  mirrorRoot?: string;
  rewriteExternalResources?: boolean;
  localizeDescriptions?: boolean;
  backend?: Web3DKitBackend;
}
...
function decodeRequestPath(request: IncomingMessage): string | null {
  try {
...
async function fileIfPresent(
  ...
  try {
    ...
    if (!realPath.startsWith(rootPrefix)) return null;
...
async function resolveMirrorFile(
  mirrorRoot: string,
  pathname: string,
): Promise<ResolvedFile | null> {
  ...
  if (requested !== mirrorRoot && !requested.startsWith(rootPrefix)) return null;
...
function etag(stats: Stats): string {
...
export function createMirrorRequestHandler(options: MirrorOptions = {}) {
  ...
  let vendorManifest: Record<string, string> | null = null;
  let translationManifest: Record<string, string> | null = null;
  const transformedCache = new Map<string, Buffer>();

  async function transformedText(path: string): Promise<Buffer | null> {
    if (!/\.(?:html|js|mjs|json)$/.test(path)) return null;
    const cached = transformedCache.get(path);
    if (cached) return cached;
    const isHtml = path.endsWith(".html");
    if (rewriteExternalResources && isHtml && vendorManifest === null) {
      try {
        vendorManifest = JSON.parse(
          await readFile(resolve(mirrorRoot, "vendor-manifest.json"), "utf8"),
    ...
    if (localizeDescriptions && translationManifest === null) {
      try {
        translationManifest = JSON.parse(
          await readFile(resolve(mirrorRoot, "i18n/es-descriptions.json"), "utf8"),
        ) as Record<string, string>;
      } catch {
        translationManifest = {};
      }
    ...
    if (backend && isHtml && !text.includes("/web3dkit-brand.css")) {
      text = text.replace(
        /<\/head>/i,
        '<link rel="stylesheet" href="/web3dkit-brand.css"></head>',
      );
    }
    if (backend && !isHtml) {
      text = text.replaceAll(
        '"https://ublctyddhtbgaersvxxb.supabase.co"',
        JSON.stringify(backend.baseUrl),
      );
      ...
      text = text.replaceAll(
        '"https://ublctyddhtbgaersvxxb.supabase.co/storage/v1/object/public/web3dkit-media"',
        '""',
      );
      text = text.replaceAll(
        '"sb_publishable_KS0-Xf-R_FOdQYOT03I1_A_MtpPvYE1"',
        '"web3dkit_local_public_key"',
      );
      text = text.replaceAll(
        '"https://web3dkit.com/api/mcp"',
        JSON.stringify(`${backend.baseUrl.replace(/\/api\/backend$/, "")}/api/mcp`),
      );
    }
    ...
    if (localizeDescriptions) {
      ...
      if (isHtml) text = text.replace(/<html\s+lang="en"/i, '<html lang="es"');
    }
  ...
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    if (backend && await backend.handle(request, response, requestUrl)) return;
...
export function createMirrorServer(options: MirrorOptions = {}) {
  ...
  if (options.backend) server.once("close", () => options.backend?.close());
  return server;
}
