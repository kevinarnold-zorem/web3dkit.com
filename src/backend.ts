...
import { sendOtpWithSmtp, type SmtpConfig } from "./smtp.js";
...
async function bodyBuffer(request: IncomingMessage, limit = 1024 * 1024): Promise<Buffer> {
  const chunks: Buffer[] = [];
  let size = 0;
...
export function backendConfigFromEnv(port: number, host: string): BackendConfig {
  const production = process.env.NODE_ENV === "production";
  const jwtSecret = process.env.AUTH_JWT_SECRET ?? (production ? "" : "web3dkit-local-development-secret-change-me");
  if (!jwtSecret) throw new Error("AUTH_JWT_SECRET is required in production");
  ...
  if (!Number.isSafeInteger(smtpPort) || smtpPort < 1 || smtpPort > 65535) throw new Error("SMTP_PORT is invalid");
  ...
  } satisfies SmtpConfig : undefined;
...
export class Web3DKitBackend {
  ...
  private catalogCache: Array<{ id: string; path: string; category: string; title: string }> | null = null;
  ...
  private verifyAccessToken(token: string): UserRow | null {
    const [header, payload, signature] = token.split(".");
    if (!header || !payload || !signature || !secureEqual(signature, hash(this.config.jwtSecret, `${header}.${payload}`))) return null;
    try {
      ...
      return (this.db.prepare("SELECT * FROM users WHERE id = ?").get(claims.sub) as unknown as UserRow | undefined) ?? null;
    } catch {
      return null;
    }
  }

  private requestUser(request: IncomingMessage): UserRow | null {
    const authorization = request.headers.authorization;
    if (!authorization?.startsWith("Bearer ")) return null;
    return this.verifyAccessToken(authorization.slice(7));
  }

  private publicUser(user: UserRow): Record<string, unknown> {
    return {
      id: user.id,
      aud: "authenticated",
      role: "authenticated",
  ...
  private createSession(user: UserRow): Record<string, unknown> {
    const refreshToken = randomBytes(32).toString("base64url");
    this.db.prepare("INSERT INTO sessions(refresh_hash,user_id,expires_at,created_at) VALUES(?,?,?,?)").run(
      hash(this.config.jwtSecret, refreshToken), user.id, new Date(Date.now() + 30 * ONE_DAY_MS).toISOString(), nowIso(),
    );
  ...
  private findOrCreateEmailUser(email: string): UserRow {
    const existing = this.db.prepare("SELECT * FROM users WHERE lower(email)=lower(?)").get(email) as unknown as UserRow | undefined;
    if (existing) {
      if (!existing.email_confirmed_at) this.db.prepare("UPDATE users SET email_confirmed_at=? WHERE id=?").run(nowIso(), existing.id);
      return (this.db.prepare("SELECT * FROM users WHERE id=?").get(existing.id) as unknown as UserRow);
    }
    const user: UserRow = { id: randomUUID(), email, email_confirmed_at: nowIso(), google_sub: null, is_anonymous: 0, created_at: nowIso() };
    this.db.prepare("INSERT INTO users(id,email,email_confirmed_at,google_sub,is_anonymous,created_at) VALUES(?,?,?,?,?,?)").run(
      user.id, user.email, user.email_confirmed_at, null, 0, user.created_at,
    );
    return user;
  }

  private async sendCode(email: string, code: string, purpose: "login" | "recovery"): Promise<void> {
    if (this.config.smtp) {
      await sendOtpWithSmtp(this.config.smtp, email, code, purpose);
      return;
    }
    if (!this.config.emailWebhookUrl) {
      if (process.env.NODE_ENV === "production") throw new Error("SMTP or EMAIL_WEBHOOK_URL is required in production");
  ...
  private googleAuthorization(redirectTo: string, linkUserId: string | null): string | null {
    if (!this.config.googleClientId || !this.config.googleClientSecret) return null;
  ...
  private entitlement(userId: string): EntitlementRow {
    return (this.db.prepare("SELECT plan,status,expires_at,stripe_customer_id FROM entitlements WHERE user_id=?").get(userId) as unknown as EntitlementRow | undefined)
      ?? { plan: "free", status: "inactive", expires_at: null, stripe_customer_id: null };
  }

  private upsertEntitlement(userId: string, plan: string, status: string, expiresAt: string | null, customerId: string | null = null): void {
  ...
  private async stripe(path: string, form?: URLSearchParams): Promise<Record<string, unknown>> {
    if (!this.config.stripeSecretKey) throw new Error("Stripe is not configured");
    ...
    if (!response.ok) throw new Error(typeof (value.error as { message?: unknown } | undefined)?.message === "string" ? String((value.error as { message: string }).message) : "Stripe request failed");
    return value;
  }

  private async stripeWebhook(request: IncomingMessage, response: ServerResponse): Promise<void> {
    if (!this.config.stripeWebhookSecret) return json(response, 503, { error: "Stripe webhook is not configured" });
    const raw = await bodyBuffer(request);
    const signature = request.headers["stripe-signature"];
    const parts = typeof signature === "string" ? Object.fromEntries(signature.split(",").map((part) => part.split("=", 2) as [string, string])) : {};
    const timestamp = parts.t;
    const received = parts.v1;
    if (!timestamp || !received || Math.abs(Date.now() / 1000 - Number(timestamp)) > 300) return json(response, 400, { error: "Invalid Stripe signature" });
    const expected = createHmac("sha256", this.config.stripeWebhookSecret).update(`${timestamp}.${raw.toString("utf8")}`).digest("hex");
    if (!secureEqual(received, expected)) return json(response, 400, { error: "Invalid Stripe signature" });
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (userId) {
        ...
        this.db.prepare(`INSERT INTO checkout_sessions(id,user_id,plan,checkout_state,payment_status,amount_total,currency,stripe_customer_id,created_at)
          VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET checkout_state='active',payment_status=excluded.payment_status,stripe_customer_id=excluded.stripe_customer_id`).run(
          id, userId, plan, "active", optionalString(object.payment_status) ?? "paid", Number(object.amount_total ?? 0), optionalString(object.currency) ?? "usd", customer, nowIso(),
        );
  ...
  private catalog(): Array<{ id: string; path: string; category: string; title: string }> {
    ...
    const items: Array<{ id: string; path: string; category: string; title: string }> = [];
    const visit = (directory: string, category: string): void => {
    ...
    for (const category of categories) visit(resolve(root, category), category);
  ...
  private async mcp(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    if (request.method !== "POST") return json(response, 405, { error: "MCP requires POST" }, { Allow: "POST, OPTIONS" });
    ...
    const send = (result: unknown): void => json(response, 200, { jsonrpc: "2.0", id, result }, { "Access-Control-Allow-Origin": "*" });
    ...
    if (method === "tools/call") {
      ...
      if (name === "search_catalog") {
        ...
        if (!item) return json(response, 200, { jsonrpc: "2.0", id, error: { code: -32602, message: "Catalog item not found" } });
        if (name === "get_catalog_item") value = { ...item, url: new URL(item.path, this.config.publicUrl).toString() };
        else if (name === "get_item_source") {
          const root = resolve(this.config.mirrorRoot ?? process.env.MIRROR_ROOT ?? "web3dkit-public");
          const storedSource = resolve(root, "source-code", `${item.id}.json`);
          let source: string;
          try { source = readFileSync(storedSource, "utf8"); } catch { source = readFileSync(resolve(root, item.path.slice(1), "index.html"), "utf8"); }
          value = { item, source };
        } else if (name === "get_item_prompt") value = { item, prompt: `Implementa ${item.title} de Web3DKit conservando su estructura, interacción, estados responsive y recursos locales. Ruta de referencia: ${item.path}.` };
        else return json(response, 200, { jsonrpc: "2.0", id, error: { code: -32601, message: "Unknown tool" } });
      }
  ...
  private async auth(request: IncomingMessage, response: ServerResponse, path: string, url: URL): Promise<boolean> {
    if (path === "/auth/v1/otp" && request.method === "POST") {
      const body = await bodyJson(request);
      const email = optionalString(body.email)?.toLowerCase();
      if (!email || !/^\S+@\S+\.\S+$/.test(email)) return json(response, 400, { code: "invalid_email", message: "Invalid email" }), true;
      const code = String(randomInt(100000, 1000000));
      this.db.prepare("INSERT INTO otp_codes(email,code_hash,expires_at,attempts) VALUES(?,?,?,0) ON CONFLICT(email) DO UPDATE SET code_hash=excluded.code_hash,expires_at=excluded.expires_at,attempts=0").run(
        email, hash(this.config.jwtSecret, code), new Date(Date.now() + 10 * 60 * 1000).toISOString(),
      );
    ...
    if (path === "/auth/v1/verify" && request.method === "POST") {
      ...
      const row = email ? this.db.prepare("SELECT * FROM otp_codes WHERE email=?").get(email) as { code_hash: string; expires_at: string; attempts: number } | undefined : undefined;
      if (!email || !token || !row || Date.parse(row.expires_at) <= Date.now() || row.attempts >= 5 || !secureEqual(row.code_hash, hash(this.config.jwtSecret, token))) {
        if (email && row) this.db.prepare("UPDATE otp_codes SET attempts=attempts+1 WHERE email=?").run(email);
    ...
    if (path === "/auth/v1/signup" && request.method === "POST") {
      const user: UserRow = { id: randomUUID(), email: null, email_confirmed_at: null, google_sub: null, is_anonymous: 1, created_at: nowIso() };
    ...
    if (path === "/auth/v1/token" && request.method === "POST" && url.searchParams.get("grant_type") === "refresh_token") {
      ...
      const row = refreshToken ? this.db.prepare("SELECT user_id,expires_at FROM sessions WHERE refresh_hash=?").get(refreshHash) as { user_id: string; expires_at: string } | undefined : undefined;
      const user = row && Date.parse(row.expires_at) > Date.now() ? this.db.prepare("SELECT * FROM users WHERE id=?").get(row.user_id) as unknown as UserRow | undefined : undefined;
      if (!user) return json(response, 401, { code: "refresh_token_not_found", message: "Invalid refresh token" }), true;
    ...
    if (path === "/auth/v1/logout" && request.method === "POST") {
      const user = this.requestUser(request);
      if (user) this.db.prepare("DELETE FROM sessions WHERE user_id=?").run(user.id);
      json(response, 204, null);
      return true;
    }
    if (path === "/auth/v1/authorize" && request.method === "GET") {
      if (url.searchParams.get("provider") !== "google") return json(response, 400, { code: "unsupported_provider", message: "Only Google is supported" }), true;
      const redirectTo = this.safeRedirect(url.searchParams.get("redirect_to"));
      const google = this.googleAuthorization(redirectTo, null);
      if (!google) return json(response, 503, { code: "google_not_configured", message: "Google login is not configured" }), true;
      redirect(response, google);
      return true;
    }
    if (path === "/auth/v1/user/identities/authorize" && request.method === "GET") {
      const user = this.requestUser(request);
      if (!user) return json(response, 401, { code: "bad_jwt", message: "Authentication required" }), true;
      if (url.searchParams.get("provider") !== "google") return json(response, 400, { code: "unsupported_provider", message: "Only Google is supported" }), true;
    ...
    if (path === "/auth/google/callback" && request.method === "GET") {
      ...
      const record = this.db.prepare("SELECT * FROM oauth_states WHERE state=?").get(state) as { redirect_to: string; link_user_id: string | null; expires_at: string } | undefined;
      if (!record || Date.parse(record.expires_at) <= Date.now() || !code || !this.config.googleClientId || !this.config.googleClientSecret) return json(response, 400, { code: "invalid_oauth_state", message: "OAuth state is invalid" }), true;
      ...
      if (!tokenResponse.ok || !token.access_token) return json(response, 401, { code: "oauth_exchange_failed", message: "Google authentication failed" }), true;
      const profileResponse = await fetch("https://openidconnect.googleapis.com/v1/userinfo", { headers: { Authorization: `Bearer ${token.access_token}` } });
      const profile = await profileResponse.json() as { sub?: string; email?: string; email_verified?: boolean };
      if (!profileResponse.ok || !profile.sub || !profile.email || !profile.email_verified) return json(response, 401, { code: "unverified_email", message: "Google did not return a verified email" }), true;
      let user: UserRow;
      if (record.link_user_id) {
        ...
        user = this.db.prepare("SELECT * FROM users WHERE id=?").get(record.link_user_id) as unknown as UserRow;
      } else {
        ...
        user = this.db.prepare("SELECT * FROM users WHERE id=?").get(user.id) as unknown as UserRow;
      }
      const session = this.createSession(user);
  ...
  private async rpc(request: IncomingMessage, response: ServerResponse, name: string): Promise<void> {
    const body = await bodyJson(request);
    if (name === "get_catalog_popularity") {
      const ids = Array.isArray(body.p_shader_ids) ? body.p_shader_ids.filter((value): value is string => typeof value === "string") : [];
      const statement = this.db.prepare("SELECT views,copies FROM shader_metrics WHERE shader_id=?");
      json(response, 200, ids.map((shaderId) => ({ shader_id: shaderId, ...((statement.get(shaderId) as { views: number; copies: number } | undefined) ?? { views: 0, copies: 0 }) })));
      return;
    }
    if (name === "get_shader_metrics") {
      const shaderId = optionalString(body.p_shader_id) ?? "";
      json(response, 200, (this.db.prepare("SELECT views,copies FROM shader_metrics WHERE shader_id=?").get(shaderId) as { views: number; copies: number } | undefined) ?? { views: 0, copies: 0 });
      return;
    }
    if (name === "get_my_entitlement") {
      const user = this.requestUser(request);
      if (!user) return json(response, 401, { code: "PGRST301", message: "JWT required" });
  ...
  private async functionCall(request: IncomingMessage, response: ServerResponse, name: string): Promise<void> {
    const body = await bodyJson(request);
    const user = this.requestUser(request);
    if (name === "shader-metrics") {
      ...
      if (!shaderId || !metric || !visitorId) return json(response, 400, { error: "Invalid metric payload" });
      ...
      if (accepted) this.db.prepare(`UPDATE shader_metrics SET ${metric === "view" ? "views" : "copies"}=${metric === "view" ? "views" : "copies"}+1 WHERE shader_id=?`).run(shaderId);
      const metrics = this.db.prepare("SELECT views,copies FROM shader_metrics WHERE shader_id=?").get(shaderId) as { views: number; copies: number };
    ...
    if (name === "send-feedback") {
      ...
      if (!message || message.length > 5000) return json(response, 400, { error: "Invalid feedback" });
    ...
    if (name === "claim-pro-entitlement") {
      ...
      if (action === "recover") {
        if (!user.email || !user.email_confirmed_at) return json(response, 200, { recovered: false });
        const purchase = this.db.prepare(`SELECT id,plan,expires_at,stripe_customer_id FROM purchases
          ...
          ORDER BY CASE plan WHEN 'lifetime' THEN 0 ELSE 1 END LIMIT 1`).get(user.email) as {
            id: string;
            plan: string;
            expires_at: string | null;
            stripe_customer_id: string | null;
      ...
      if (action === "begin") {
        if (!user.is_anonymous) return json(response, 400, { error: "Anonymous purchase session required" });
      ...
      if (action === "complete") {
        const token = optionalString(body.token);
        const claim = token ? this.db.prepare("SELECT * FROM claim_tokens WHERE token_hash=?").get(hash(this.config.jwtSecret, token)) as { source_user_id: string; expires_at: string } | undefined : undefined;
        if (!claim || Date.parse(claim.expires_at) <= Date.now()) return json(response, 400, { error: "Invalid claim token" });
        const source = this.entitlement(claim.source_user_id);
        if (source.plan === "free") return json(response, 404, { error: "No entitlement to claim" });
      ...
      if (action === "verify") {
        ...
        const recovery = email ? this.db.prepare("SELECT * FROM recovery_codes WHERE billing_email=?").get(email) as { code_hash: string; expires_at: string } | undefined : undefined;
        const purchase = email ? this.db.prepare("SELECT * FROM purchases WHERE lower(billing_email)=lower(?) AND status='active'").get(email) as { id: string; plan: string; expires_at: string | null; stripe_customer_id: string | null } | undefined : undefined;
        if (!email || !code || !recovery || !purchase || Date.parse(recovery.expires_at) <= Date.now() || !secureEqual(recovery.code_hash, hash(this.config.jwtSecret, code))) return json(response, 403, { error: "Invalid recovery code" });
        this.upsertEntitlement(user.id, purchase.plan, "active", purchase.expires_at, purchase.stripe_customer_id);
        this.db.prepare("UPDATE purchases SET claimed_user_id=? WHERE id=?").run(user.id, purchase.id);
    ...
    if (name === "create-checkout-session") {
      ...
      if (!plan) return json(response, 400, { error: "Invalid plan" });
      ...
      if (!price || !this.config.stripeSecretKey) return json(response, 503, { error: "Checkout is not configured" });
      ...
      if (plan === "yearly") form.set("subscription_data[metadata][user_id]", user.id);
      if (user.email) form.set("customer_email", user.email);
      const session = await this.stripe("checkout/sessions", form);
      const stripeId = optionalString(session.id);
      if (!stripeId || !optionalString(session.url)) return json(response, 502, { error: "Stripe did not return a checkout session" });
    ...
    if (name === "create-portal-session") {
      const entitlement = this.entitlement(user.id);
      if (!entitlement.stripe_customer_id) return json(response, 400, { error: "No billing customer is linked" });
    ...
    if (name === "checkout-status") {
      ...
      let local = id ? this.db.prepare("SELECT * FROM checkout_sessions WHERE id=? AND user_id=?").get(id, user.id) as Record<string, unknown> | undefined : undefined;
      if (!local) return json(response, 404, { error: "Checkout session was not found" });
      if (id?.startsWith("cs_") && this.config.stripeSecretKey && local.checkout_state !== "active") {
        ...
        if (paid) {
          ...
          local = this.db.prepare("SELECT * FROM checkout_sessions WHERE id=?").get(id) as Record<string, unknown>;
        }
      }
    ...
    if (name === "redeem-designcode-pass") {
      ...
      const pass = token ? this.db.prepare("SELECT expires_at,redeemed_by FROM designcode_passes WHERE token_hash=?").get(tokenHash) as { expires_at: string; redeemed_by: string | null } | undefined : undefined;
      if (!pass || Date.parse(pass.expires_at) <= Date.now() || pass.redeemed_by && pass.redeemed_by !== user.id) return json(response, 403, { error: "DesignCode pass is invalid or already redeemed" });
  ...
  async handle(request: IncomingMessage, response: ServerResponse, url: URL): Promise<boolean> {
    if (url.pathname === "/api/mcp") {
      try { await this.mcp(request, response); } catch (error) { json(response, 500, { error: error instanceof Error ? error.message : "MCP failed" }); }
      return true;
    }
    ...
    try {
      if (await this.auth(request, response, path, url)) return true;
      ...
      if (path === "/dev-checkout" && request.method === "GET" && this.config.devCheckout) {
        ...
        const checkout = this.db.prepare("SELECT * FROM checkout_sessions WHERE id=?").get(id) as { user_id: string; plan: string } | undefined;
        if (!checkout) return json(response, 404, { error: "Checkout not found" }), true;
