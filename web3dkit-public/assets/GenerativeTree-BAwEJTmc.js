import{r as n,j as b}from"./index-ChUl42DD.js";const A=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
`,e={speed:1,size:1,particleAmount:1,opacity:1,hue:0,saturation:1,brightness:1};function t(o,a,r){return Math.min(r,Math.max(a,o))}function k(o,a){const r=Math.max(0,Math.round(50*t(a,0,2))),s=1/t(o,.65,1.5);return A.replace(/<script[^>]+cloudflareinsights\.com[^>]*><\/script>/gi,"").replace("</head>",`<style data-generative-tree-focus>
...
<\/script></head>`).replace("const PARTICLE_COUNT = 50;",`const PARTICLE_COUNT = ${r};`).replace("const _pad = parseFloat(new URLSearchParams(location.search).get('p')) || 1;",`const _pad = ${s.toFixed(4)};`).replace(`function frame(time) {
    // Decay shake`,`function frame(time) {
    if (window.__GENERATIVE_TREE_CONTROLS.paused) { requestAnimationFrame(frame); return; }

    // Decay shake`).replace("b.growthProgress = Math.min(1, b.growthProgress + b.growthSpeed);","b.growthProgress = Math.min(1, b.growthProgress + b.growthSpeed * window.__GENERATIVE_TREE_CONTROLS.speed);").replace("holdTimer++;","holdTimer += window.__GENERATIVE_TREE_CONTROLS.speed;").replace("fadeTimer++;","fadeTimer += window.__GENERATIVE_TREE_CONTROLS.speed;").replace("waitTimer++;","waitTimer += window.__GENERATIVE_TREE_CONTROLS.speed;").replace(`  createTree();
  requestAnimationFrame(frame);`,`  function startTreeWhenSized() {
    resize();
    if (W <= 0 || H <= 0) {
  ...
  requestAnimationFrame(startTreeWhenSized);`)}function P({speed:o=e.speed,size:a=e.size,particleAmount:r=e.particleAmount,opacity:s=e.opacity,hue:h=e.hue,saturation:p=e.saturation,brightness:w=e.brightness,className:u="",style:x}){const c=n.useRef(null),[T,v]=n.useState(!0),[E,y]=n.useState(()=>typeof document>"u"||!document.hidden),l=t(o,0,3),f=!T||!E||l===0,g=n.useMemo(()=>k(a,r),[r,a]),d=n.useCallback(()=>{c.current?.contentWindow?.postMessage({type:"generative-tree-controls",controls:{speed:l,paused:f}},"*")},[f,l]);return n.useEffect(()=>{const i=c.current;if(!i||typeof IntersectionObserver>"u")return;const m=new IntersectionObserver(([S])=>v(S?.isIntersecting??!0));return m.observe(i),()=>m.disconnect()},[]),n.useEffect(()=>{if(typeof document>"u")return;const i=()=>y(!document.hidden);return document.addEventListener("visibilitychange",i),()=>document.removeEventListener("visibilitychange",i)},[]),n.useEffect(()=>{d()},[d,g]),b.jsx("div",{className:`web3dkit-background generative-tree${u?` ${u}`:""}`,style:{background:"#0a0a0a",pointerEvents:"auto",...x},children:b.jsx("iframe",{ref:c,title:"Generative Tree background",srcDoc:g,sandbox:"allow-scripts",onLoad:d,"aria-hidden":"true",tabIndex:-1,style:{position:"absolute",inset:0,display:"block",width:"100%",height:"100%",border:0,background:"#0a0a0a",opacity:t(s,.05,1),filter:`hue-rotate(${t(h,-180,180)}deg) saturate(${t(p,0,2)}) brightness(${t(w,.35,1.8)})`}})})}export{e as GENERATIVE_TREE_DEFAULTS,P as GenerativeTree};
