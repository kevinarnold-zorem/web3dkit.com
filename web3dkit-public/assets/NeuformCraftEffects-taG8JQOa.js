import{j as h,r as y}from"./index-ChUl42DD.js";const k=`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
</html>`,d={hue:0,saturation:1,brightness:1},i={neon:{title:"Animated neon typography",source:A,background:n=>n==="light"?"#f4f4f2":"#090909",targets:[{selector:"#board",role:"ui",width:"1040px"}],presentation:"animated-typography"},luminaWeaversCloth:{title:"Woven Cloth kinetic textile",source:T,background:"#16090b",targets:[{selector:"body > div.fixed.inset-0.overflow-hidden.z-0",role:"background"}],presentation:"woven-cloth-label"},julianVanceNebula:{title:"Julian Vance nebula background",source:E,background:"#09090b",targets:[{selector:"#bg-canvas",role:"background"}]},fluid:{title:"Aura UI fluid background",source:_,background:"#030306",targets:[{selector:"#bg-canvas",role:"background"}]},nexusUnifiedFlow:{title:"Nexus unified halftone flow",source:C,background:"#000000",targets:[{selector:"#glcanvas",role:"background"}]},emberStorm:{title:"Aeonix ember storm",source:k,background:"#080503",targets:[{selector:"#gl",role:"background"}]},engravedCertificate:{title:"Kinetic Lathe certificate",source:S,background:"#ded6c2",targets:[{selector:"#cert",role:"ui",width:"720px"}]}};function p(n,e,t){return Math.min(t,Math.max(e,n))}function u(n,e,t){if(!n.includes(e))throw new Error(`Neuform source adapter could not find: ${e}`);return n.replace(e,t)}function P(n){return[["return { t0: after + 5.6 + frnd() * 7.2, dur: total, segs: segs };","return { t0: after + 1.25 + frnd() * 1.75, dur: total, segs: segs };"],["if (!ev) ev = nextEvent(4.2);","if (!ev) ev = nextEvent(0.55);"],["cap = Math.min((W - W * 0.12) / blockW, (H - H * 0.18) / blockH);","cap = Math.min((W - W * 0.20) / blockW, (H - H * 0.28) / blockH);"],["var y0 = (H - blockH * cap) * 0.48;","var y0 = (H - blockH * cap) / 2;"]].reduce((e,[t,a])=>u(e,t,a),n)}function R(n){return[["x.fillText('L W', W/2, 190);","x.fillText('W C', W/2, 190);"],["x.fillText('· KYOTO ·', W/2, 246);","x.fillText('· WOVEN CLOTH ·', W/2, 246);"],["x.fillText('LUMINA', W/2, 400);","x.fillText('WOVEN', W/2, 400);"],["x.fillText('WEAVERS', W/2, 520);","x.fillText('CLOTH', W/2, 520);"],["x.fillText('K I N E T I C   T E X T I L E S   ·   2 0 2 4', W/2, 626);","x.fillText('T E X T I L E   S I M U L A T I O N', W/2, 626);"]].reduce((e,[t,a])=>u(e,t,a),n)}function b(n,e){return typeof n.background=="function"?n.background(e):n.background}function M(n,e){const t=JSON.stringify(n.targets).replace(/</g,"\\u003c"),a=b(n,e),s=e==="light"?"grayscale(1) invert(1) contrast(1.08)":"grayscale(1) contrast(1.08)",l=n.presentation==="animated-typography"?`
...
${l}
</style>`,o=`<script data-web3dkit-focus>
(function () {
  var isolated = false;
  function isolate() {
    if (isolated) return;
...
<\/script>`;return(n.presentation==="animated-typography"?P(n.source):n.presentation==="woven-cloth-label"?R(n.source):n.source).replace(/<\/head>/i,`${c}</head>`).replace(/<\/body>/i,`${o}</body>`)}function j({definition:n,mode:e="dark",hue:t=d.hue,saturation:a=d.saturation,brightness:s=d.brightness,className:l,style:c}){const o=e==="light"?"light":"dark",x=b(n,o),v=y.useMemo(()=>M(n,o),[n,o]),f=p(t,-180,180),g=p(a,0,2),m=p(s,.35,1.65),w=f===0&&g===1&&m===1?void 0:`hue-rotate(${f}deg) saturate(${g}) brightness(${m})`;return h.jsx("iframe",{className:l,"data-mode":o,title:n.title,srcDoc:v,sandbox:"allow-scripts",loading:"eager",style:{display:"block",width:"100%",height:"100%",border:0,background:x,filter:w,...c}})}function r(n){return function(t){return h.jsx(j,{...t,definition:n})}}const H=r(i.neon),W=r(i.luminaWeaversCloth),I=r(i.julianVanceNebula),z=r(i.fluid),D=r(i.nexusUnifiedFlow),F=r(i.emberStorm),B=r(i.engravedCertificate);export{F as EmberStorm,B as EngravedCertificate,z as FluidFieldBackground,D as HalftoneFlow,d as NEUFORM_CRAFT_DEFAULTS,I as NebulaBackground,H as NeonTypography,W as WovenCloth};
