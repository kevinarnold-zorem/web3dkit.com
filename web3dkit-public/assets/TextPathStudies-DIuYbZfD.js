import{j as a,r as D}from"./index-ChUl42DD.js";const u=`<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
`,s={mode:"dark",scale:1,opacity:1,hue:0,saturation:1,brightness:1},e={globe:{source:h,index:1,title:"Globe",layout:"figures"},outlineTypeflow:{source:h,index:2,title:"Outline Typeflow",layout:"figures",renderDensity:{minimum:2,maximum:2.5}},morphingGlyphCloud:{source:h,index:3,title:"Morphing Glyph Cloud",layout:"figures",renderDensity:{minimum:2,maximum:2.5}},cloth:{source:h,index:6,title:"Cloth",layout:"figures"},ripple:{source:u,index:2,title:"Ripple",layout:"cards",presentation:"dark-frameless"},ball:{source:u,index:3,title:"Ball",layout:"cards",presentation:"dark-frameless"}},z={"globe-study":e.globe,"outline-typeflow":e.outlineTypeflow,"morphing-glyph-cloud":e.morphingGlyphCloud,"cloth-study":e.cloth,"ripple-study":e.ripple,"ball-study":e.ball};function c(A,n,t){return Math.min(t,Math.max(n,A))}function S(A,n){const t=A.index,r=n==="light"?"#f3f5f8":"#08090a",o=n==="light"?`
      :root {
        color-scheme: light;
        --bg: #f3f5f8;
    ...
    `:":root { color-scheme: dark; }",l=A.presentation==="dark-frameless"?`
      .stage, .row {
    ...
    `:`
      ...
      ${l}
    `;return`<style id="web3dkit-study-focus">
    ${o}
    html, body, .frame {
      width: 100% !important;
      height: 100% !important;
    ...
    body { margin: 0 !important; background: ${r} !important; }
    .frame { padding: 0 !important; background: ${r} !important; }
    header { display: none !important; }
    ${p}
  </style>`}const R="var dpr = Math.min(1.5, window.devicePixelRatio || 1);",w="var INK  = '20,19,16';",B="var INK  = '226,228,233';";function y(A,n,t){if(!A.includes(n))throw new Error(`Text path source adapter could not find: ${n}`);return A.replace(n,t)}function E(A,n){return[[w,n==="light"?w:"var INK  = '238,240,244';"],[`      var cell = s.w * 0.02860;
      var fs   = cell * 1.10;`,`      var size = Math.min(s.w, s.h);
      var cell = size * 0.02860;
      var fs   = cell * 1.10;`],["      var speed = s.w * 1.20;","      var speed = size * 1.20;"],[`      var fs = s.w*FS;
      var now = performance.now();
      var hitR = s.w*0.055, hit = hitR*hitR;`,`      var size = Math.min(s.w, s.h);
      var fs = size*FS;
      var now = performance.now();
      var hitR = size*0.055, hit = hitR*hitR;`],[`          vx: (dx/d)*s.w*(0.10 + Math.random()*0.16) + (Math.random()-0.5)*s.w*0.05,
          vy: (dy/d)*s.w*0.08 - s.w*(0.04 + Math.random()*0.10),`,`          vx: (dx/d)*size*(0.10 + Math.random()*0.16) + (Math.random()-0.5)*size*0.05,
          vy: (dy/d)*size*0.08 - size*(0.04 + Math.random()*0.10),`],[`      var cx = s.w/2, cy = s.h/2 + s.w*CYOFF;
      var R  = s.w*RR;
      var fs = s.w*FS;`,`      var size = Math.min(s.w, s.h);
      var cx = s.w/2, cy = s.h/2 + size*CYOFF;
      var R  = size*RR;
      var fs = size*FS;`],["      var g = s.w*0.80;","      var g = size*0.80;"]].reduce((r,[o,l])=>y(r,o,l),A)}function H(A,n){const t=A.presentation==="dark-frameless"?E(A.source,n):n==="light"?y(A.source,B,"var INK  = '38,40,48';"):A.source;return(A.renderDensity?t.replace(R,`var dpr = Math.min(${A.renderDensity.maximum}, Math.max(${A.renderDensity.minimum}, window.devicePixelRatio || 1));`):t).replace(/<title>[\s\S]*?<\/title>/i,`<title>${A.title} — Web3DKit</title>`).replace("</head>",`${S(A,n)}
</head>`)}function i({definition:A,mode:n=s.mode,scale:t=s.scale,opacity:r=s.opacity,hue:o=s.hue,saturation:l=s.saturation,brightness:p=s.brightness,className:m,style:b}){const d=n==="light"?"light":"dark",M=D.useMemo(()=>H(A,d),[A,d]),v=c(t,.65,1.5),P=c(r,.1,1),f=c(o,-180,180),g=c(l,0,2),x=c(p,.4,1.8),k=f===0&&g===1&&x===1?void 0:`hue-rotate(${f}deg) saturate(${g}) brightness(${x})`;return a.jsx("div",{className:["text-path-study",`text-path-study--${d}`,m].filter(Boolean).join(" "),"data-mode":d,style:{opacity:P,filter:k,...b},children:a.jsx("iframe",{className:"text-path-study-frame","data-mode":d,title:`${A.title} interactive canvas study`,sandbox:"allow-scripts",srcDoc:M,style:{transform:v===1?void 0:`scale(${v})`}})})}function T({variant:A="globe-study",...n}={}){return a.jsx(i,{definition:z[A],...n})}function I(A={}){return a.jsx(i,{definition:e.globe,...A})}function L(A={}){return a.jsx(i,{definition:e.outlineTypeflow,...A})}function j(A={}){return a.jsx(i,{definition:e.morphingGlyphCloud,...A})}function q(A={}){return a.jsx(i,{definition:e.cloth,...A})}function F(A={}){return a.jsx(i,{definition:e.ripple,...A})}function O(A={}){return a.jsx(i,{definition:e.ball,...A})}export{O as BallStudy,q as ClothStudy,I as GlobeStudy,j as MorphingGlyphCloud,L as OutlineTypeflow,F as RippleStudy,s as TEXT_PATH_STUDY_DEFAULTS,T as TextPathStudies};
