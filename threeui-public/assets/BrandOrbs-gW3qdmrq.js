import{r as a,j as L}from"./index-ChUl42DD.js";const R=`<!doctype html>
...
`,B=["claude","openai","codex","cursor","gemini","figma","framer","react","swift","designcode","aura","dreamcut","ui","ux","css","ios","neuform","github","x","instagram","threads","linkedin","email"],T=["small","medium"],o={variant:"claude",size:"medium",mode:"dark",speed:1,paused:!1},P={small:20,medium:56},w={claude:"Claude Code",openai:"OpenAI",codex:"Codex",cursor:"Cursor",gemini:"Gemini",figma:"Figma",framer:"Framer",react:"React",swift:"Swift",designcode:"DesignCode",aura:"Aura",dreamcut:"DreamCut",ui:"UI",ux:"UX",css:"CSS",ios:"iOS",neuform:"Neuform",github:"GitHub",x:"X",instagram:"Instagram",threads:"Threads",linkedin:"LinkedIn",email:"Email"};function O(n,e,t){return Math.min(t,Math.max(e,n))}function b(){if(typeof document>"u"||typeof window>"u")return"dark";const n=document.documentElement.dataset.theme??document.documentElement.dataset.scheme;return n==="light"||n==="dark"?n:document.documentElement.classList.contains("light")?"light":document.documentElement.classList.contains("dark")||window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"}function _(n){const[e,t]=a.useState(b);return a.useEffect(()=>{if(!n||typeof document>"u"||typeof window>"u")return;const c=window.matchMedia("(prefers-color-scheme: dark)"),s=()=>t(b()),i=new MutationObserver(s);return i.observe(document.documentElement,{attributes:!0,attributeFilter:["class","data-scheme","data-theme"]}),c.addEventListener("change",s),s(),()=>{i.disconnect(),c.removeEventListener("change",s)}},[n]),e}function D(n){return[...n.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/gi)].at(-1)?.[1]??""}const H=D(R).replace(/<\/script/gi,"<\\/script");function j(n,e,t){const c=t==="light"?"#dad7cc":"#050608",s=t==="light"?"invert(1) hue-rotate(180deg) contrast(1.04) saturate(.92)":"none",i=P[e],d=JSON.stringify(n).replace(/</g,"\\u003c"),p=`<script data-brand-orbs-controls>
(function () {
...
<\/script>`,r=H.replace('if (document.visibilityState !== "hidden")','if (document.visibilityState !== "hidden" && !window.__BRAND_ORB_PAUSED)');return`<!doctype html>
<html lang="en" data-theme="${t}">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>${w[n]} Brand Orb</title>
<style>
html, body { width: 100%; height: 100%; margin: 0; overflow: hidden; background: ${c}; }
body { display: grid; place-items: center; }
canvas { display: block; width: ${i}px; height: ${i}px; filter: ${s}; }
</style>
${p}
</head>
<body>
<canvas data-mode=${d} data-size="${i}" aria-hidden="true"></canvas>
<script>${r}<\/script>
</body>
</html>`}function N({variant:n=o.variant,size:e=o.size,mode:t=o.mode,speed:c=o.speed,paused:s=o.paused,"aria-label":i,className:d,style:p}){const r=a.useRef(null),[k,M]=a.useState(!0),[z,C]=a.useState(()=>typeof document>"u"||!document.hidden),S=_(t==="auto"),h=t==="auto"?S:t,g=B.includes(n)?n:o.variant,m=T.includes(e)?e:o.size,x=O(c,.1,3),u=s||!k||!z,f=a.useMemo(()=>j(g,m,h),[h,m,g]),A=h==="light"?"#dad7cc":"#050608",v=a.useCallback(()=>{r.current?.contentWindow?.postMessage({type:"brand-orbs-controls",controls:{speed:x,paused:u}},"*")},[u,x]);return a.useEffect(()=>{const l=r.current;if(!l||typeof IntersectionObserver>"u")return;const y=new IntersectionObserver(([E])=>M(E?.isIntersecting??!0));return y.observe(l),()=>y.disconnect()},[]),a.useEffect(()=>{if(typeof document>"u")return;const l=()=>C(!document.hidden);return document.addEventListener("visibilitychange",l),()=>document.removeEventListener("visibilitychange",l)},[]),a.useEffect(()=>{v()},[v,f]),L.jsx("iframe",{ref:r,className:d,title:i??`${w[g]} animated brand orb`,srcDoc:f,sandbox:"allow-scripts",loading:"eager",onLoad:v,style:{display:"block",width:"100%",height:"100%",border:0,background:A,...p}})}export{o as BRAND_ORBS_DEFAULTS,T as BRAND_ORB_SIZES,B as BRAND_ORB_VARIANTS,N as BrandOrbs};
